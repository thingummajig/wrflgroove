...
def track_list(request, dj_name):
    ...
    queryset = Playlist.objects.filter(dj=DJ(id=deejay.id)).order_by('-playtime')
    return object_list(request, queryset=queryset, paginate_by = 25, extra_context = { 'deejay': deejay })
